...
let ranks = ["normal", "altin", "elmas", "hazir", "topluluk", "api", "kodlar"];
for (let rank in ranks) {
  db[ranks[rank]] = new bookman(ranks[rank]);
}
//Crew Code #Hyper
const IDler = {
manyak: "822842673232085032",
  botID: "780528005922684948",
  botToken: "NzgwNTI4MDA1OTIyNjg0OTQ4.X7wZWg.Hsou5zDjWcZP0SFUGYtgApmaeS0",
  botSecret: "l1D-QQotO1aNVUlc3fyc4W2A1tPYLeje",
  botCallbackURL: "https://crewcodesite.glitch.me/callback",
  sunucuID: "744679033282887795",
  sunucuDavet: "https://discord.gg/invite/zdpUEuvsmx",
  kodLogKanalı: "839428879638724658",
  sahipRolü: "811139447516692510",
  adminRolü: "837985263044001823",
  kodPaylaşımcıRolü: "839429174686908416",
  boosterRolü: "770180271277735936",
  kodPaylaşamayacakRoller: ["822842675292143626"],
  hazırAltyapılarRolü: "837992455252213810",
  hazırSistemlerRolü: "837992455252213810",
  elmasKodlarRolü: "837992455252213810",
  altınKodlarRolü: "837992455252213810",
  normalKodlarRolü: "837992455252213810"
};
    ...
    return res.redirect(
      url.format({
        ...
        query: {
          statuscode: 137,
          message:
            "Kodlarımızı görebilmeniz için sunucumuza katılmanız lazım | Griş yapmanız lazım"
        }
      })
    );
...
app.get("/altin/:id", (req, res) => {
 ...
 member.roles.cache.has(IDler.manyak))
    ) {
      res.render("kod", {
        user: req.user,
        kod: code
      ...
      res.redirect(
        url.format({
          ...
          query: {
            statuscode: 501,
            message: "Bu Kodu Görmeye Malasef Yetkiniz Yetmemektedir."
          }
        })
      );
    }
  ...
  if (!member)
    res.redirect(
      url.format({
        ...
        query: {
          statuscode: 501,
          message: "Sitemizde böyle bi profile ait kişi bulunamadı."
        }
      })
    );
  else {
        ...
        if (!id) {
          res.redirect(
            url.format({
              ...
              query: {
                statuscode: 504,
                message: "İstediğiniz kodun ıd sini belirtin"
              }
            })
          );
        }
        ...
        if (!rank) {
          res.redirect(
            url.format({
              ...
              query: {
                statuscode: 504,
                message: "Kodun Hangi rank içinde olacağını belirleyin."
              }
            })
          );
        }
...
member.roles.cache.has(IDler.manyak)
  ...
  client.channels.cache.get(IDler.kodLogKanalı).send(
    ...
    .addField("> Kod Sayfası", `[Tıkla!](https://crewcodesite.glitch.me/${obj.kod_rank}/${obj.id})`));
